...
def _rescale_and_clean(masks, image, labels):
    ...
    masks = F.interpolate(masks, size=image.size()
...
def balanced_mask_loss_ce(mask, pseudo_gt, gt_labels, ignore_index=4, old_classes=16):
    ...
    mask = F.interpolate(mask, size=pseudo_gt.size()
                         [-2:], mode="bilinear", align_corners=True)
    ...
    class_weight = (num_pixels_total - num_pixels_per_class) / \
        (1 + num_pixels_total)  # BS, C
    class_weight = (
        pseudo_gt * class_weight[:, :, None, None]).sum(1).view(bs, -1)  # BS, H, W

    # BCE loss
    loss = F.cross_entropy(
        mask, mask_gt, ignore_index=ignore_index, reduction="none")
    loss = loss.view(bs, -1)
...
def balanced_mask_loss_unce(mask, pseudo_gt, gt_labels, old_cl, ignore_index=255):
    ...
    # because we are equally interested in all classes
    bs, c, h, w = pseudo_gt.size()
    num_pixels_per_class = pseudo_gt.view(
        bs, c, -1).sum(-1)  # BS, C -> pixel per class
    # BS -> pixel per image
    num_pixels_total = num_pixels_per_class.sum(-1, keepdim=True)
    class_weight = (num_pixels_total - num_pixels_per_class) / \
        (1 + num_pixels_total)  # BS, C
    ...
    outputs[:, 0] = torch.logsumexp(
        mask[:, 0:old_cl], dim=1) - den  # B, H, W       p(O)
    outputs[:, old_cl:] = mask[:, old_cl:] - \
        den.unsqueeze(dim=1)  # B, N, H, W    p(N_i)

    loss = F.nll_loss(outputs, mask_gt, ignore_index=ignore_index,
...
#             nn.Conv2d(ch_sh, ch_skip, 1, bias=False), norm(ch_skip))
#         self.fc8_x = nn.Sequential(nn.Conv2d(
#             ch_skip+channels, 256, kernel_size=3, stride=1, padding=1, bias=False), norm(256))
...
#                                        nn.Dropout(0.5),
#                                        nn.Conv2d(
#                                            256, 256, kernel_size=3, stride=1, padding=1, bias=False), norm(256),
#                                        nn.Dropout(0.1))
#         self.init_pars()
...
#             pos_weight=torch.full([classes], fill_value=pos_weight))
#         self.fusion = 'mean'
...
#     def init_pars(self):
#         modules = [self.fc8_skip, self.fc8_x,
#                    self.head, self.last_conv, self.cls]
...
#         # == MASKS PREDICTION
#         # only #class as channels (no background predicted)
#         logits = self.compute_masks(features)
...
#         # focal penalty loss
#         y_focal = torch.pow(1 - masks_.mean(-1), 3) * \
#             torch.log(0.01 + masks_.mean(-1))

#         # adding the losses together
#         # background excluded from the loss (not trained)
#         y = y_ngwp[:, 1:] + y_focal[:, 1:]
...
#             im = F.interpolate(image_raw, masks.detach().size()
#                                [-2:], mode="bilinear", align_corners=True)
...
#             masks = F.interpolate(masks, size=out_size,
