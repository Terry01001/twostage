...
def valid_pseudo(model, pseudolabeler, dataloader, test_num_pos, device):
    step = 0
    num = 0
    total_f = 0
    nclass = 4
    ...
    with torch.no_grad():
        for sample in dataloader:
            ...
            pred = F.interpolate(preds[-2], size=(label.shape[-1], label.shape[-1]), mode='bilinear', align_corners=False)
        ...
        Pseudo_Acc = evaluator.Pixel_Accuracy()
        Pseudo_Acc_class = evaluator.Pixel_Accuracy_Class()
        Pseudo_mIoU = evaluator.Mean_Intersection_over_Union()
        Pseudo_ious = evaluator.Intersection_over_Union()
        Pseudo_FWIoU = evaluator.Frequency_Weighted_Intersection_over_Union()

        return {'Pseudo_Acc':Pseudo_Acc, 
                'Pseudo_Acc_class':Pseudo_Acc_class, 
                'Pseudo_mIoU':Pseudo_mIoU, 
                'Pseudo_ious':Pseudo_ious, 
                'Pseudo_FWIoU':Pseudo_FWIoU}