...
class ToTensor(object):
    ...
    def __call__(self, pic, lbl=None):
        ...
        """
        # pic = np.array(pic).astype(np.float32).transpose(2, 0, 1)
        # print(type(pic[0]))
        # pic = np.asarray(pic)
        # print(pic)
        if lbl is not None:
            if isinstance(lbl, torch.Tensor):
                return F.to_tensor(pic), lbl   #F.to_tensor(pic)
            else:
                return F.to_tensor(pic), torch.from_numpy(np.array(lbl, dtype=np.uint8))#torch.from_numpy(np.array(lbl, dtype=np.float32)).permute(2, 0, 1)
        else:
